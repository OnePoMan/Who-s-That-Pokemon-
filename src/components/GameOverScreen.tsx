...
export default function GameOverScreen({ winner, players, roundResults, onPlayAgain, onShare }: GameOverScreenProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full max-w-md mx-auto animate-fade-in">
      ...
      <h2 className="font-pixel text-base text-pokemon-gold text-center">
      ...
      <div className="flex gap-6 items-center">
        {players.map((player) => (
          <div key={player.id} className={`flex flex-col items-center gap-1.5 ${player.id === winner.id ? 'scale-110' : 'opacity-70'}`}>
            ...
            {player.id === winner.id && (
              <span className="text-[10px] font-bold text-pokemon-gold bg-yellow-100 px-2 py-0.5 rounded-full font-body">Winner!</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="w-full space-y-2">
        <h3 className="text-xs font-bold text-pokemon-dark font-body uppercase tracking-wide">Round History</h3>

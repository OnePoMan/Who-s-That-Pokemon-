...
export default function DrawingGallery({ roundResults, onClose }: DrawingGalleryProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 animate-fade-in">
      <div className="bg-pokemon-cream rounded-2xl shadow-2xl border-4 border-pokemon-dark w-full max-w-2xl max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b-2 border-pokemon-dark bg-gradient-to-r from-pokemon-red to-pokemon-red-dark rounded-t-xl">
          <h2 className="font-pixel text-xs text-white">Drawing Gallery</h2>
          <button
            onClick={onClose}
                ...
                <div key={i} className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                  ...
                  <div className="p-2 space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-pokemon-dark truncate font-body">{result.pokemon.name}</span>
                    ...
                    <p className="text-[10px] text-pokemon-gray font-body">by {result.drawer.nickname}</p>

                    {/* Download button */}
                    {result.drawingDataUrl && (
                      <button
                        onClick={() => handleDownload(result)}
                        className="w-full mt-1 py-1 rounded-lg text-[10px] font-bold font-body text-pokemon-blue bg-blue-50 hover:bg-blue-100 transition-colors border border-pokemon-blue/20"
                      >
                        Download
                      </button>
                    )}
